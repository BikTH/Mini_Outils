...
/**
 * importQuestionsFromPdf(int $examId, string $pdfPath) : int
 * - Lecture PDF en PHP pur (Smalot\PdfParser), pas de shell_exec.
 * - Parse les blocs "QUESTION NO:" / options A. B. C. ... / Correct Answer:
 * - Stocke explication (texte après la ligne Correct Answer:).
...
function importQuestionsFromPdf(int $examId, string $pdfPath): int
{
    ...
    // Découper sur "QUESTION NO:" (insensible à la casse)
    $blocks = preg_split('/QUESTION\s*NO:\s*/i', $content);
    $importedCount = 0;

    // Le premier bloc avant la première question est souvent vide/intro
    foreach ($blocks as $block) {
        ...
        foreach ($lines as $line) {
            ...
            // Après la réponse, tout est explication
            if ($seenAnswer) {
                $explicationLines[] = $line;
                continue;
            }

            // Sinon, on reste dans l'énoncé
            $enonceLines[] = $line;
        }
        ...
        $pdo = getPDO();
        // Insérer la question avec explication
        $stmt = $pdo->prepare(
